\chapter{Testing ed Evidenze}
    ...
    Di seguito vengono formalizzate le metriche utilizzate e il loro significato nel contesto del progetto.
        ...
        \subsection{Latenza di Elaborazione (Processing Latency)}
        ...
        \subsection{Throughput Sostenibile (Packets Per Second - PPS)}
        ...
        \subsection{Occupazione della Coda (Queue Load Saturation)}
        ...
        Dato che il campionamento periodico potrebbe non catturare picchi istantanei di riempimento, il software è stato strumentato per registrare il \textbf{Picco di Occupazione} (\textit{Peak Queue Load}) raggiunto nell'intervallo di campionamento.
        ...
        \subsection{Accuratezza del Rilevamento (Detection Rate)}
    ...
    Per ottenere le metriche definite nella sezione 6.1 senza alterare significativamente le prestazioni del sistema (riducendo al minimo l'effetto osservatore), è stato necessario "attrezzare" il codice sorgente.
    ...
    L'obiettivo è dimostrare empiricamente come la soluzione proposta scali all'aumentare del carico e come le scelte architetturali impattino sulla latenza e sulla resilienza del servizio.
        ...
        \begin{itemize}
            \item \textbf{Gruppo 1 (1.a/1.b):} Serve a stabilire la \textit{baseline} delle prestazioni. Utilizzando un solo thread, ci aspettiamo di evidenziare i limiti fisici dell'elaborazione sequenziale e il fenomeno della saturazione della coda.
            \item \textbf{Gruppo 2 (2.a/2.b):} Introduce il parallelismo. Confrontando questi risultati con il Gruppo 1, si mira a quantificare il guadagno prestazionale (speedup) e la riduzione della latenza garantita dall'architettura Produttore-Consumatore.
            \item \textbf{Gruppo 3 (3.a/3.b/3.c):} Simula un ambiente reale e ostile. Qui si valuta la capacità del sistema di gestire interfacce multiple e la sua resilienza (robustezza) quando una parte della rete è sotto attacco Denial of Service (DoS), verificando che il servizio sulle altre interfacce non venga interrotto.
        \end{itemize}
    ...
    Per ciascuno scenario, verrà inoltre discussa l'efficacia del dimensionamento della coda (\texttt{QUEUE\_SIZE}), confrontando i risultati ottenuti con la dimensione standard (1000 slot) rispetto a quella maggiorata (10000 slot), al fine di valutare il trade-off tra utilizzo della memoria e latenza di accodamento.
        ...
        In questo primo gruppo di test, il router DAI è stato configurato con un singolo thread di analisi e una coda standard (\texttt{ARP\_QUEUE\_SIZE = 1000}). Questo scenario rappresenta la \textit{baseline} per comprendere i limiti intrinseci dell'elaborazione sequenziale.
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=\textwidth]{immagini/testing_plot/1a_legit.png}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=\textwidth]{immagini/testing_plot/1b_print.png}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=\textwidth]{immagini/testing_plot/1b_no_print.png}
...
% ----------------- Dimensione Coda Maggiore --------------------            
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=\textwidth]{immagini/testing_plot/1a10.png}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=\textwidth]{immagini/testing_plot/1b10.png}
            ...
            In conclusione, il tuning della coda risolve il problema del \textit{packet loss} ma non quello della latenza, che rimane vincolata alla capacità computazionale del singolo core.
        ...
        Il secondo gruppo di test segna il passaggio all'architettura parallela. Mantenendo invariato il carico di traffico (50.000 pacchetti), il router è stato configurato con un pool di \textbf{4 thread analizzatori} concorrenti. Questa configurazione mira a verificare se la parallelizzazione del processo di consumo possa eliminare il collo di bottiglia osservato in precedenza.
            ...
            Il dato più significativo riguarda la latenza media (grafico viola), che crolla dai valori millimetrici del test precedente a circa \textbf{13-15 $\mu$s}. Si tratta di una riduzione di due ordini di grandezza (circa 100 volte più veloce), che rende il processo di ispezione trasparente e privo di impatti percepibili sulla rete.

            \begin{figure}[H]
                \centering
                \includegraphics[width=0.9\textwidth]{immagini/testing_plot/2a.png}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.9\textwidth]{immagini/testing_plot/2b.png}
                \caption{Scenario 2.b: Efficienza del rilevamento sotto attacco flood (4 Thread).}
                \label{fig:2b}
            \end{figure}

            Il grafico evidenzia come la linea degli attacchi rilevati (verde) cresca linearmente e senza interruzioni fino al totale previsto, mentre la latenza rimane ancorata ai minimi di (\textbf{$\sim$6-7 $\mu$s}). L'assenza di saturazione della coda garantisce che nessun pacchetto venga scartato all'ingresso, assicurando una copertura di sicurezza del 100\% anche durante tentativi di \textit{Flooding Attack}.
...
% ----------------- Dimensione Coda Maggiore --------------------
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.9\textwidth]{immagini/testing_plot/2a10.png}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.9\textwidth]{immagini/testing_plot/2b10.png}
            ...
            A differenza dello scenario 1.a (dove la coda lunga causava latenze di 6 ms), qui la latenza media rimane eccellente (\textbf{$\sim$23 $\mu$s} per il traffico legittimo e addirittura \textbf{$\sim$6.5 $\mu$s} sotto attacco).
        ...
        L'ultimo gruppo di test introduce il livello di complessità più elevato, simulando un ambiente di rete reale in cui il router deve gestire contemporaneamente traffico proveniente da interfacce fisiche distinte (\texttt{enp0s8} e \texttt{enp0s9}). L'obiettivo è verificare la capacità del sistema di isolare i flussi di traffico e prevenire che un attacco su una sottorete comprometta il servizio sull'altra.
            ...
            Nonostante la contemporaneità degli eventi, la latenza media rimane contenuta entro i \textbf{19 $\mu$s}, confermando che l'attività malevola su un'interfaccia non degrada le prestazioni globali del router.

            \begin{figure}[H]
                \centering
                \includegraphics[width=0.9\textwidth]{immagini/testing_plot/3a.png}
                \caption{Scenario 3.a: Gestione concorrente di traffico legittimo (Rete 1) e attacco (Rete 2).}
                \label{fig:3a}
            \end{figure}
            ...
            Ciò dimostra che, anche in condizioni di saturazione parziale del buffer, l'architettura multi-thread è in grado di processare e salvare il traffico lecito, garantendo la continuità del servizio (\textit{Service Availability}) anche sotto assedio.

            \begin{figure}[H]
                \centering
                \includegraphics[width=0.9\textwidth]{immagini/testing_plot/3b.png}
...
% ----------------- Dimensione Coda Maggiore --------------------
            ...
            Come evidenziato nella Figura \ref{fig:3b10}, nello scenario di resistenza al DoS, l'utilizzo della coda crolla dal 100\% (del test standard) a un mero \textbf{11.59\%}.

            Ancora più sorprendente è il risultato dello stress test massimo (Figura \ref{fig:3c10}): anche con 100.000 pacchetti in ingresso simultaneo, il buffer non supera mai il \textbf{17.24\%} di occupazione.

            Questo risultato sancisce che la configurazione \textbf{4 Thread + Coda 10k} rappresenta l'assetto ottimale per l'ambiente di produzione, capace di trasformare potenziali situazioni di disservizio (coda piena) in eventi di ordinaria gestione, mantenendo la latenza media sotto la soglia critica dei \textbf{10 $\mu$s} nella maggior parte degli scenari operativi.
            \begin{figure}[H]
                \centering
                \includegraphics[width=\textwidth]{immagini/testing_plot/3a10.png}
                \caption{3.b (Coda 10k): DoS Resistance senza saturazione.}
                \label{fig:3b10}
            \end{figure}
            
            \begin{figure}[H]
                \centering
                \includegraphics[width=\textwidth]{immagini/testing_plot/3b10.png}
                \caption{3.c (Coda 10k): Heavy Load con coda al 17\%.}
                \label{fig:3c10}
            \end{figure}
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=\textwidth]{immagini/testing_plot/3c.png}
                \caption{Scenario 3.c: Throughput massimo (34k PPS) sotto carico estremo.}
                \label{fig:3c}
            \end{figure}

            In questo contesto estremo, si osserva un fisiologico aumento della latenza media (\textbf{$\sim$270 $\mu$s}), imputabile alla contesa (\textit{lock contention}) tra i due thread ricevitori e i quattro analizzatori che accedono contemporaneamente alla coda condivisa. Tuttavia, il sistema mantiene la stabilità operativa, processando correttamente il 100\% del carico (50k attacchi bloccati e 50k pacchetti leciti inoltrati) senza perdite.
